...
import { fadeUp,
  ...
  floatLoop,
  hoverScale,
  staggerContainer,} from "../animations/animation";
// import {
//   fadeUp,
//   fadeDown,
//   scaleIn,
...
// ─────────────────────────────────────────
//  Floating background items (unchanged)
// ─────────────────────────────────────────
const floatingItems = [
  { src: "/coffee.webp",      left: "8%",   size: 34, duration: 7,    delay: 0,   rotate: -20, rotateEnd: 20,  opacity: 0.5  },
  { src: "/coffee11.webp",    left: "9%",   size: 34, duration: 7,    delay: 0,   rotate: -20, rotateEnd: 20,  opacity: 0.5  },
...
export default function Hero() {
  ...
  // ── Cup shared render ──────────────────
  const CupInner = ({ desktop = false }: { desktop?: boolean }) => (
    <div
      style={{
        position: "relative",
        width: desktop ? "clamp(400px, 42vw, 340px)" : "clamp(310px, 60vw, 400px)",
        userSelect: "none",
        cursor: "pointer",
      }}
    >
      ...
      {/* Lid */}
      <div
        className={`lid ${hovered ? "lid-hovered" : "lid-normal"}`}
      ...
      <div
        style={{
          ...
          left: "50%",
          transform: "translateX(-50%)",
          width: desktop ? "200px" : "150px",
          height: desktop ? "32px" : "28px",
          borderRadius: "50%",
          background: "var(--color-rock-gold, #C9913A)",
          opacity: 0.28,
          filter: `blur(${desktop ? 28 : 22}px)`,
          zIndex: 1,
        }}
      />
    </div>
  ...
  return (
    <>
      <style>{`
        ...
        [data-theme="light"] .hero-tree-branch::after {
          background: linear-gradient(to left, rgba(245,240,232,0.0) 0%, rgba(245,240,232,0.55) 55%, rgba(245,240,232,0.88) 100%);
        }
        ...
        .hero-overlay-dir {
          position: absolute;
          inset: 0;
          background: linear-gradient(to right, rgba(13,11,8,0.92) 0%, rgba(13,11,8,0.55) 45%, rgba(13,11,8,0.08) 100%);
          pointer-events: none;
        }
        [data-theme="light"] .hero-overlay-dir {
          background: linear-gradient(to right, rgba(245,240,232,0.96) 0%, rgba(245,240,232,0.65) 45%, rgba(245,240,232,0.1) 100%);
        }

        /* ── Bottom fade ── */
        .hero-fade-bottom {
          position: absolute;
          inset-inline: 0; bottom: 10px;
          height: 200px;
          background: linear-gradient(to top, var(--bg-primary, #080808), transparent);
          pointer-events: none;
          z-index: 5;
        ...
        [data-theme="light"] .hero-sub { color: var(--color-rock-ink-soft, #2D2B29); }
        ...
        [data-theme="light"] .hero-btn-ghost { border-color: rgba(28,26,24,0.25); color: var(--color-rock-ink, #1C1A18); }
        [data-theme="light"] .hero-btn-ghost:hover { border-color: var(--color-rock-amber, #D4872A); color: var(--color-rock-amber, #D4872A); }
        ...
        [data-theme="light"] .sidebar-pill { background: rgba(28,26,24,0.05); border-color: rgba(28,26,24,0.13); }
        .sidebar-dot { width: 6px; height: 6px; border-radius: 50%; background: #4ade80; flex-shrink: 0; }
        .sidebar-text {
          writing-mode: vertical-rl;
          text-orientation: mixed;
          transform: rotate(180deg);
        ...
        [data-theme="light"] .sidebar-text { color: var(--color-rock-ink-soft, #2D2B29); }
        ...
        /* ── Mobile cup ── */
        .hero-mobile-cup-abs { display: block; }
        .hero-desktop-cup    { display: none; }

        /* ══════════════════════════════════
           DESKTOP (≥ 900px)
        ══════════════════════════════════ */
        @media (min-width: 900px) {
          .hero-tree-branch { display: block !important; }
          ...
          .hero-desktop-left .hero-sub { font-size: 1rem !important; text-align: left !important; max-width: 440px !important; }
          .hero-desktop-left .hero-btns { width: auto !important; max-width: none !important; }
          .hero-desktop-left .hero-btn-primary,
          .hero-desktop-left .hero-btn-ghost {
            flex: 0 0 auto !important;
            padding: 14px 28px !important;
      ...
      >
        ...
        <div className="hero-desktop-cup">
          ...
          >
            {/* Floating loop wrapper (sits inside entrance wrapper) */}
            <motion.div animate={floatLoop}>
              <CupInner desktop={true} />
            </motion.div>
          </motion.div>
        </div>
      </section>
